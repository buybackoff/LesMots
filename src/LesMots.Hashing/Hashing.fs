...
[<AutoOpenAttribute>]
module Hashing = 
    let hash (word : string) : List<int * int * string> =        
                ...
                if subLength > 1 then 
                    ...
                    for i in 0..word.Length - subLength do // moving beginning of substring
                        ...
                        let ok, subCode = 
                            ...
                            | 4 -> Chars.Quads.TryGetValue(sub)
                            | 3 -> Chars.Triples.TryGetValue(sub)
                            | 2 -> Chars.Tuples.TryGetValue(sub)
                    ...
                    for c in charArray do
                        let sub = String(c, 1)
                        let ok, subCode = Chars.Singles.TryGetValue(sub)
